...
class UserForm(forms.Form):
    ...
    def clean_name(self):
        # плохой пример подмены параметра min_length
        name = self.cleaned_data['name']
        if len(name) < 3:
            raise forms.ValidationError('Имя должно быть больше 3 символов')
        return name

    def clean_email(self):
        email: str = self.cleaned_data['email']
        if not (email.endswith('vk.team') or email.endswith('corp.mail.ru')):
            raise forms.ValidationError('Используйте корректную почту')
        return email
